...
fn main() {
    ...
    loop {
        ...
        if verbose {
            ...
            let Some(parts) = RE.captures(&equ) else {
                todo!()
            };
            println!(
                "lhs: {} op: {} rhs: {}",
                &parts["lhs"], &parts["op"], &parts["rhs"]
            );
        }
        ...
        if verbose {
            println!("Var to index: {:?}", var_to_indx);
        }
        ...
        for (_, [var]) in VAR_RE.captures_iter(&tr_vars).map(|c| c.extract()) {
            let pow_val = match var_to_indx.get(var) {
                Some(v) => v,
                None => {
                    println!("Incorect variable name when defining always true variables.\n Problematic name: {}", var);
                    return;
                }
            };
        ...
        let mut table_struct = bool_table::new(
            verbose,
            equ,
            tr_val_int,
            charset,
